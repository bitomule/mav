...
// The deduction is OFF unless asked for, and this is the test that keeps it
// off. Measured: on the route it was built for it answered `none` 10 times out
// of 10, and each of those was a model round trip (~350 ms) spent on every
// single `goto` to learn nothing. A fixed cost for a benefit that did not
// appear does not go on the default path.
func TestTheDeductionIsOffUnlessAskedFor(t *testing.T) {
	args := []string{"the box contents"}
	if hasFlag(args, "--infer-arrival") {
		t.Fatal("the deduction would run with nobody asking for it")
	}
	if !hasFlag([]string{"the box contents", "--infer-arrival"}, "--infer-arrival") {
		t.Fatal("the flag does not turn it on")
	}
	// And it must not eat the goal: a bare flag is not a positional word.
	goal := gotoPositional([]string{"the", "box", "--infer-arrival", "contents"})
	if len(goal) != 3 {
		t.Fatalf("the flag was read as part of the goal: %v", goal)
	}
}
