...
an error. Lint the matrix before running it — a bad value found at step 12 costs
...
`mav sim appearance` returns only after the screen has repainted (~1.5s), because
the capture path otherwise serves the pre-switch frame and the dark cell of the
matrix comes out light. No `delay` step is needed between it and the capture.
